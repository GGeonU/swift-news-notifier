<p align="center">
  ...
  <img src="https://cdn-icons-png.flaticon.com/512/8943/8943377.png" width="60" alt="AI" />
...
* Swift/iOS 영문 아티클을 AI로 요약해서 Slack으로 알림받는 자동화 서버

## 📌 프로젝트 개요

### 문제 인식
- Swift/iOS 아티클을 제공하는 유명한 Repository https://github.com/SAllen0400/swift-news 
...
### 프로젝트 목표
1. Nest.js 프레임워크 학습: 실제 입력과 출력 명세를 가진 백엔드 개발 프로젝트 진행해보기
...
- **Slack Web API**: Slack API를 활용한 메시지 전송
...
### 전체 흐름
프로젝트는 3가지 방식으로 실행됩니다: 
  1. 매일 9시 자동 실행,
  2. Slack Command를 통한 수동 체크
  3. 특정 URL 요약 요청
...
### 2. 실제 동작에 대한 테스트가 어려움 → 테스트 코드 적극 도입
...
- 총 19개의 테스트 코드 작성, 특히 주요 로직들에 대한 테스트 코드 작성 & 검증
    * Git diff 내역에서 URL을 뽑아내는 로직
    * Gemini를 통해 요약된 내용을 올바르게 Entity로 변환하는 로직
    * 요약된 Summary를 Slack에 보내는 메시지 블록으로 변환하는 로직

### 3. AI 도구 적극 활용
- 단순히 코드를 작성하게 시키는게 아닌 전반적인 방향성을 공유할 수 있도록, CLAUDE.md 작성해 컨텍스트를 주입
- 질문도 단순히 코드 작성을 시키는게 아닌, 최대한 많은 정보를 고려할 수있도록 질문

```
// 모듈 의존성이 깊어지면서 생기는 문제 해결

모듈간의 의존성이 깊은 뎁스로 생기면 어떤 부분이 문제가 될 수 있어?
Nest.js 에서는 이를 어떤 방식들로 해결하는지, 트레이드오프를 고려해서 현재 상황에 적용할 수 있는 해결책을 고려해줘.
```

* 실제 요약 로직 적용시에도 질문을 최대한 구체적으로 해서 원하는 결과를 얻을 수 있도록 프롬프팅
* 처음에도 일반적으로는 잘 요약이 되었으나, 요청 마다 답변의 내용이나 형식이 조금씩 달라지는 경우가 있었음 (예시 이외의 텍스트가 더 추가된다거나, 요약 내용의 뉘앙스가 조금씩 달라지는)
* 요약 형식의 Preset 제공, 예외 케이스 추가 (URL 파싱 불가), 더 디테일하게 단계적인 요구 사항 명세 등을 추가해서
* 더 일관적이고 의미있는 응답이 올 수 있도록 프롬프팅 진행
```
// 최초 아티클 요약 프롬프팅

당신은 Swift/iOS 기술 아티클 요약 전문가입니다.
**중요: 아래 URL의 아티클을 읽고, 핵심만 간단히 요약하세요!**

**주요 내용 (1-2줄):**
- 기술 용어는 영어로 유지
- 중요한 포인트를 추출해서 한국어로 번역
...
<img width="769" height="329" alt="image" src="https://github.com/user-attachments/assets/0eb2be31-880b-41d0-8275-ee1ac5bf391e" />
...
* URL의 내용이 Swift/iOS 관련 내용이 아닌 경우

<img width="823" height="547" alt="image" src="https://github.com/user-attachments/assets/5eedaa6e-064c-4bdc-bef9-f998ad4d0365" />
